...
outfile = open('positions' + '-' + repr(year) + repr(month).zfill(2) + repr(day).zfill(2) + repr(hour).zfill(2) + '.gpx', 'w')
outfile.write("""<?xml version="1.0" encoding="UTF-8" standalone="no" ?>

<gpx xmlns="http://www.topografix.com/GPX/1/1" creator="RNS850" version="1.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">
  <metadata>
    <time>""")
...
outfile.write(repr(year) + "-" + repr(month).zfill(2) + "-" + repr(day).zfill(2) + "T" + repr(hour).zfill(2) + ":" + repr(minutes).zfill(2) + ":" + repr(seconds).zfill(2) + """Z</time>
  ...
  <trk>
    <name>RNS850 """ + repr(year) + repr(month).zfill(2) + repr(day).zfill(2) + repr(hour).zfill(2) + """</name>
    <trkseg>
""")
...
#	timestampdate = int(c,16)
	...
	longitude = float(int(b[15] + b[14] + b[13] + b[12],16)) / 1000000
